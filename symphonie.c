 ...
 *g_build_filename () builds the full path depending on the operating system. (/
 *for Linux and \ for Windows) Loading the window.glade file Assignment of
 *interface signals to different CallBacks Recovering the pointer from the
 *software loading popup window Closing the loading popup and then displaying
 *the main window
 */
int main(int argc, char *argv[]) {
  ...
  start_window = (GtkWindow *)GTK_WIDGET(
      gtk_builder_get_object(data.builder, "StartWindow"));
  ...
  main_window = (GtkWindow *)GTK_WIDGET(
      gtk_builder_get_object(data.builder, "MainWindow"));
 ...
 *We hide the dialog window. If we destroy it the next call to this callback
 *would cause a segdefault!
 */
...
void on_itemPreference_activate(GtkMenuItem *itemPreference,
  ...
  dialog =
      GTK_WIDGET(gtk_builder_get_object(data->builder, "PreferenceWindow"));
  gtk_dialog_run(GTK_DIALOG(dialog));
...
void on_SearchEntry_search_changed() {
  ...
  if (mot[0] != '\0') {
    ...
    if (!string_phon) return;

    GtkLabel *phonetisation = (GtkLabel *)GTK_WIDGET(
        gtk_builder_get_object(data.builder, "phonetisation"));
...
void on_setNumberResultsButton_clicked() {
  GtkSpinButton *spin_button = (GtkSpinButton *)GTK_WIDGET(
      gtk_builder_get_object(data.builder, "spinNumber"));
  nbResults = gtk_spin_button_get_value_as_int(spin_button);
  GtkWindow *preference_window = (GtkWindow *)GTK_WIDGET(
      gtk_builder_get_object(data.builder, "PreferenceWindow"));
...
void deleteChildren() {
  ...
  flow_box = (GtkContainer *)GTK_WIDGET(
      gtk_builder_get_object(data.builder, "FlowBox"));
