...
void* list_find(List* list, void* object) {
  ...
  if (!list->_lock) {
    ...
    if (med < list->_count && list->_compare(list->_data[med], object) == 0) {
      output = list->_data[med];
    }
  }
  return output;
...
void* list_get(List* list, int index) {
  if (index < 0 || index >= list->_count) return NULL;
...
int list_count(List* list) { return list->_count; }
...
void list_unlock(List* list) { list->_lock = false; }

bool list_is_locked(List* list) { return list->_lock; }
...
void list_destroy(List* list, _freeFunc elm_free) {
  if (elm_free) {
    ...
    fprintf(stderr,
            "WARN. List: destroying a list without freeing the elements");
  }
  free(list->_data);
  free(list);
}
...
List* list_new(_compFunc function) {
  ...
  if (list) {
    ...
    if (filled) list->_compare = function;
    return list;
